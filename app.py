...
# --- Registration (Using Google Form Bridge - No Permission Needed) ---
        ...
        if st.form_submit_button("SAVE DATA"):
            if name and phone:
                ...
                try:
                    ...
                    st.error("Technical error, please try again.")
            else:
                st.warning("Naam aur Number likhein.")

# --- Search Page (Read-Only CSV Mode) ---
else:
    ...
    gid = "2137978586" # Aapki sheet ka GID number
            ...
            for i, row in df[::-1].iterrows():
                st.markdown(f"""
                    <div class="donor-card">
                        ...
                        <a href="tel:{row.iloc[4]}" style="background:#28a745; color:white; padding:10px; text-decoration:none; border-radius:8px; display:block; text-align:center;">📞 CALL NOW</a>
                    </div>
                """, unsafe_allow_html=True)
        else:
            st.info("Abhi koi donor nahi hai.")
    except:
        st.info("Data load ho raha hai... Sheet check karein.")
