...
# Main Header
st.markdown('<div class="header-box"><h1>PUNJAB BLOOD DONATION</h1><p>Welfare Committee Pindi Amolak    Create By Rana Mani</p></div>', unsafe_allow_html=True)
            ...
            if not f_df.empty:
                for i, row in f_df[::-1].iterrows():
                    ...
                    <div class="donor-card">
                        <h3 style="margin:0; color:#990000;">{row.iloc[1]}</h3>
                        <p style="margin:5px 0;">🩸 <b>{row.iloc[2]}</b> | 📍 {row.iloc[3]}</p>
                        <div class="number-display">{row.iloc[4]}</div>
                        <a href="tel:{row.iloc[4]}" class="call-btn">📞 CALL ME</a>
                    </div>
                    """, unsafe_allow_html=True)
            else:
                st.info(f"No donors available for {blood_choice}.")
        else:
            st.warning("No records found.")
    except:
        st.info("Loading donors...")
